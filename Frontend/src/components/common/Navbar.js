...
const NavBar = () => {
  ...
  const fetchSublinks = async () => {
    try {
      ...
      // console.log(result?.data?.allCategories?.length);
  ...
  return (
    ...
    >
      <div className="flex w-11/12 max-w-maxContent items-center justify-between ">
        <Link to="/">
          <img src={logo} width={150} alt="Learn Hub" height={38}></img>
        </Link>
              ...
              <li key={index}>
                {element.title === "Catalog" ? (
                  <div className=" flex items-center group relative cursor-pointer">
                    ...
                    <div className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 gap-2 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]">
                        ...
                        sublinks?.map((element, index) => (
                          <Link
                            to={`/catalog/${element?.name
                              ...
                              .toLowerCase()}`}
                            key={index}
                            className="rounded-lg bg-richblack-25 py-4 pl-4 hover:bg-richblack-50"
                          >
